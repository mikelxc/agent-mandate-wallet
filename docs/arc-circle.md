...
`packages/contracts/script/DeployKernelArc.s.sol` is guarded to chain 5042002, the authorized deployer, canonical EntryPoint code hash, Circle domain and six-decimal USDC. Its dry run succeeded before broadcast. `bun scripts/record-arc-deployment.ts` rechecks public evidence without signing or broadcasting. `packages/subgraph/subgraph.arc-testnet.yaml` targets the deployed registry starting at block 61600603; Studio version `wayleave/arc-testnet-v0.1.0` is deployed, with its first indexed account registration reconciled against the Arc receipt. The index is still backfilling; see [subgraph evidence](../deployments/subgraph-arc-testnet.json). Arc MCP/gateway configuration and decentralized-network publication remain separate steps.
