  ...
  if (!address)
    return (
      <div className="flow-surface">
        ...
        <p>
          Connect your owner wallet to load its ENS names and agent wallets.
        </p>
...
function OwnedAccessPicker({ owner }: { owner: Address }) {
      ...
      setErrors(
        results.flatMap((result, i) =>
          result.status === 'rejected'
            ? [
                `${i ? 'Arc' : 'Sepolia'} wallets could not be loaded. Retry to refresh ownership.`,
              ]
            : [],
        ),
      );
      const requested = found.find(
        (wallet) =>
          wallet.account.toLowerCase() === requestedAccount?.toLowerCase() &&
          String(wallet.chainId) === requestedChain,
      );
  ...
  if (loading)
    return <p role="status">Loading ENS names from your owned wallets…</p>;
  ...
  if (requestedAccount && !requested)
    return (
      <div className="flow-surface">
        <p role="alert">
          The selected wallet could not be verified as yours on that network.
        </p>
  ...
  return (
    <>
      ...
      {!identities.length && (
        <p>
          No owned Sepolia names were found. You can verify another ENS identity
          below.
        </p>
      )}
      <AgentAccessFlow
        key={`${identity?.key ?? 'manual'}:${arc?.account ?? ''}`}
        restoreExisting
        initialName={identity ? agentEnsName(identity.name) : undefined}
        initialAccount={arc?.account}
