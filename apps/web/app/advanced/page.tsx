...
export default function AdvancedWorkspace() {
  const [tab, setTab] = useState<'preview' | 'live' | 'kernel' | 'agents'>(
    'agents',
  );
  ...
  const tabMeta = {
    agents: {
      ...
      title: 'Agent control',
      description:
        'Connect an agent, understand its reach, and approve every payment.',
    },
    kernel: {
    ...
    <div className="workspace">
      <aside className="sidebar">
        ...
        <nav className="side-nav" aria-label="Workspace">
          ...
          >
            <Bot size={17} /> Agent control <ChevronRight size={14} />
          </button>
          <button
            className={`nav-item ${tab === 'kernel' ? 'active' : ''}`}
            onClick={() => setTab('kernel')}
          >
            <Wallet size={17} /> Sepolia account <ChevronRight size={14} />
          </button>
          <button
            className={`nav-item ${tab === 'preview' ? 'active' : ''}`}
            onClick={() => setTab('preview')}
          >
            <TestTube2 size={17} /> Policy playground <ChevronRight size={14} />
          </button>
          <button
            className={`nav-item ${tab === 'live' ? 'active' : ''}`}
            onClick={() => setTab('live')}
          >
            <Blocks size={17} /> Contracts <ChevronRight size={14} />
          </button>
        </nav>
        <div className="sidebar-note">
          ...
          <div>
            ...
            <p>Agents can read and propose. Only you can approve.</p>
          </div>
        </div>
        <div className="sidebar-footer">
          Sepolia prototype <span className="network-dot" />
        </div>
      </aside>
      <main>
        <header>
          <span className="breadcrumb">
            Mandate <ChevronRight size={13} /> {tabMeta.title}
          </span>
          <button
            className="secondary"
            onClick={() =>
        ...
        <section className="content">
          ...
          {tab === 'agents' ? (
            <AgentControl key={`agent-${wallet.address}-${wallet.chainId}`} />
          ) : tab === 'kernel' ? (
            <>
              <PasskeyOnboarding />
                  ...
                  {tab === 'preview' ? (
                    ...
                    <>
                      ...
                      <p className="footnote">
                        ...
                        token units. Only the account owner can grant authority.
                      </p>
                    </>
                  )}
                </section>
